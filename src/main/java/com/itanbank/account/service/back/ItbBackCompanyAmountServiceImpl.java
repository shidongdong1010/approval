...
@Transactional
@Service
public class ItbBackCompanyAmountServiceImpl implements ItbBackAmountService {
    @Autowired
    ...
    private boolean isBack(Long userId, Long projectId, ItbInvestInfo investInfo, double amount){
        List<ItbOrderInfo> list = itbOrderInfoRepository.findPayInByCompany(userId, projectId, BusinessTypeEnum.REPAY_BACK.getCode());
        if(list != null) {
    ...
    public ProjectTransferResult addOrderInfo(ItbProjectInfo projectInfo, ItbProjectAccount projectAccount, ItbInvestInfo investInfo, String ip) throws Exception{
        ItbCompanyInfo user = itbCompanyInfoService.findById(investInfo.getUserId());
        ...
        orderInfo.setUserType(UserTypeEnum.ENTERPRISE.getCode()); // 用户类型
        ...
        logger.info("返款到企业投资者, 保存订单{}", orderInfo);
    ...
    public ExecuetResultCode back(ItbOrderInfo orderInfo, ItbProjectInfo projectInfo, ProjectTransferResult rojectTransferResult) throws Exception{
        ...
        ItbCompanyInfo companyInfo = itbCompanyInfoService.findById(orderInfo.getUserId()); // 查询企业投资者用户账户
        ...
        if(OrderStatusEnum.PAYMENT.equals(orderInfo.getOrderStatus())) {
            logger.error("返款到企业投资者业务处理失败, 订单目前状态不是支付中。 订单信息：{}", orderInfo);
    ...
    private void backFailure(ItbOrderInfo orderInfo, String tradeNo, ItbProjectInfo projectInfo, String retMsg) throws Exception {
        ...
        if(!isPass){
            logger.error("返款到企业投资者通知结果失败，更新订单表失败。 订单信息：{}", orderInfo);
            throw new Exception("返款到企业投资者通知结果失败，更新订单表失败。 订单信息：" + orderInfo);
        }
        logger.info("返款到企业投资者通知结果失败，业务处理完成。 失败原因：{}， 订单信息：{}", retMsg, orderInfo);
    }
    ...
    private void backSuccess(ItbOrderInfo orderInfo, String tradeNo, ItbCompanyInfo companyInfo, ItbProjectInfo projectInfo, ItbProjectAccount projectAccount , String retMsg) throws Exception{
        ...
        boolean isPass = itbOrderInfoService.updateOrderByIdAndVersion(tradeNo, orderStatus, orderInfo.getId(), orderInfo.getVersion());
        if(!isPass){
            logger.error("返款到企业投资者通知结果成功，更新订单表失败。 订单信息：{}", orderInfo);
            throw new Exception("返款到企业投资者通知结果成功，更新订单表失败。 订单信息：" + orderInfo);
        }
        // 修改标的账户余额
        ...
        if(!isPass){
            logger.error("返款到企业投资者通知结果成功，更新标的账户表失败。 订单信息：{}", orderInfo);
            throw new Exception("返款到企业投资者通知结果成功，更新标的账户表失败。 订单信息：" + orderInfo);
        }
        // 修改企业账户余额
        isPass = itbCompanyInfoService.updateBalanceByIdAndVersion(orderInfo.getAmount(), companyInfo.getId(), companyInfo.getVersion());
        if(!isPass){
            logger.error("返款到企业投资者通知结果成功，更新企业账户表失败。 订单信息：{}", orderInfo);
            throw new Exception("返款到企业投资者通知结果成功，更新企业账户表失败。 订单信息：" + orderInfo);
        }
    ...
    private ItbBackUserInfo getItbBackUserInfo(ItbOrderInfo orderInfo, ItbProjectInfo projectInfo){
        ...
        if(backUserInfo == null){
            ...
            backUserInfo.setUserType(UserTypeEnum.ENTERPRISE.getCode());
