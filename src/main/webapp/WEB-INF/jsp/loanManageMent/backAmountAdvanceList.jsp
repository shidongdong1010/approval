...
<body>
	...
	function gridReload() {
		...
		jQuery("#grid-table").jqGrid('setGridParam', {
			url : "backAmountAdvanceList.json",
			mtype : "post",
			page : 1,
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				...
				mtype : "post",
				height: 330,
				colNames:['标的Id','借款人姓名','借款人手机','借款合同编号', '借款合同金额', '标的名称', '招的结束时间','垫付本金','垫付利息','违约金','垫付还款本金','垫付还款利息','垫付还款违约金','状态','操作'],
				colModel:[
					{
						name:'id',
						index:'id',
					...
					{
						name:'duetoLateRepayServerAmount',
			...
			*/
			...
			function openInvestDialog(id){
				...
				jQuery(invest_grid_selector).jqGrid({
					url : 'backAdvance.json',
					datatype : "json",
					postData: {projectId: id},
					mtype : "post",
					height: '180',
					width: '640',
					colNames:['垫付本金', '垫付利息', '违约金', '已返本金', '已返利息', '已返违约金','返款状态'],
					colModel:[
						{
							name:'repayInfo.advanceCapital',
							index:'advanceCapital',
							width:100,
							editable: false
						}, {
							name:'repayInfo.advanceInterest',
							index:'advanceInterest',
							width:80,
							editable: false
						}, {
							name: 'repayInfo.duetoReapyPenalty',
							index:'duetoReapyPenalty',
							width:100,
							editable: false
						},{
							name: 'backAdvanceInfo.repayCapital',
							index:'repayCapital',
							width:100,
							editable: false
						},{
							name: 'backAdvanceInfo.repayInterest',
							index:'repayInterest',
							width:100,
							editable: false
						},{
							name: 'backAdvanceInfo.penaltyAmount',
							index:'penaltyAmount',
							width:100,
							editable: false
						},
						{
							name:'backAdvanceInfo.status',
							index:'status',
							width:100,
							editable: false,
							formatter:function(cellvalue, options, rowObject){
					...
					buttons: [
						{
							...
							click: function() {
								...
								$.post("${ctx}/backAmountAdvance.json", data, function(msg){
									if(msg.code != 200){
