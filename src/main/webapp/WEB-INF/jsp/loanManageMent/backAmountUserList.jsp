	...
	function gridReload() {
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				...
				mtype : "post",
				height: 330,
				colNames:['标的Id','借款人姓名','借款人手机','借款合同编号', '借款合同金额', '标的名称', '招的结束时间','状态','操作'],
				colModel:[
					{
						name:'id',
						index:'id',
			...
			*/
			...
			function openInvestDialog(id){
				...
				jQuery(invest_grid_selector).jqGrid({
					url : 'backInsertList.json',
					datatype : "json",
					postData: {projectId: id},
					mtype : "post",
					height: '180',
					width: '600',
					colNames:['投资人姓名', '投资金额', '收益率(年)', '预期收益', '已返本金', '已返收益','返款状态'],
					colModel:[
						{
							name:'investInfo.userName',
							index:'userName',
							width:120,
							editable: false
						},
						{
							name:'investInfo.amount',
							index:'amount',
							width:100,
							editable: false,
							summaryType: 'sum'
						...
						{
							name:'investInfo.returnRate',
							index:'returnRate',
							width:80,
							editable: false
						},
						{
							name:'investInfo.returnAmount',
							index:'returnAmount',
							width:100,
							editable: false
						},{
							name: 'backUserInfo.repayCapital',
							index:'repayCapital',
							width:100,
							editable: false
						},{
							name: 'backUserInfo.returnAmount',
							index:'repayCapital',
							width:100,
							editable: false
						},
						{
							name:'backUserInfo.status',
							index:'status',
							width:100,
							editable: false,
							formatter:function(cellvalue, options, rowObject){
					...
					buttons: [
						{
							...
							click: function() {
								...
								$.post("${ctx}/backAmountUser.json", data, function(msg){
									if(msg.code != 200){
