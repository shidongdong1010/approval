...
<body>
	...
	function gridReload() {
		...
		jQuery("#grid-table").jqGrid('setGridParam', {
			url : "discountList.json",
			mtype : "post",
			page : 1,
			postData: {loanName:loanName, loanContractNo: loanContractNo}
		}).trigger("reloadGrid");
	}
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				...
				mtype : "post",
				height: 330,
				colNames:['标的Id','借款人姓名','借款人手机','借款合同编号', '借款合同金额', '标的名称', '开标时间', '招标结束时间','标的状态'],
				colModel:[
					{
						name:'id',
						index:'id',
			...
			jQuery(grid_selector)
					...
					.navButtonAdd(pager_selector,{
						caption:"贴现",
						title: "贴现",
						buttonicon:"icon-share-alt",
						onClickButton: payAmount
					});
			/// 对表格式样式进行修改
			...
			*/
			...
			function payAmount(){
				...
				bootbox.confirm({  
			        ...
			        message: queryBalance(selectedIds[0],'0'),  
			        callback: function(result) {  
			            if(result) {  
			            	$.post("discount",{id:selectedIds[0]},function(msg){
								if(msg.code=='200'){
